...
func (c *callback) keyboardCallback(nCode int, wparam w32.WPARAM, lParam w32.LPARAM) w32.LRESULT {
	if nCode == 0 && wparam == w32.WM_KEYDOWN {
		...
		case w32.VK_RETURN:
			...
			level.Info(c.logger).Log("process", c.activeProcess, "title", c.activeWindow, "words", words, "backspace_count", c.backCounter, "duration", dur, "wpm", wpm, "text", c.currentText)
			c.currentText = ""
			c.backCounter = 0
		case w32.VK_CONTROL:
			c.checkTimer()
